...
# YCSB sweep for aria, ariaer, and ariaer_cache_fix (Aria paper settings)
# - threads: 12
# - partitions: 12
...
ARIAER_CACHE_FIX="${ROOT}/ariaer_cache_fix/bench_ycsb"
POSTPROCESS_SCRIPT="${SCRIPT_DIR}/ycsb_logs_to_csv.sh"
RESULTS_ROOT="${RESULTS_ROOT:-${ROOT}/result}"
...
case "${1:-run}" in
  run)
    ...
    run_engine "${ARIAER_CACHE_FIX}" "ariaer_cache_fix" "Aria" || failed=1
    run_engine "${ARIAER_CACHE_FIX}" "ariaer_cache_fix_ariafb" "AriaFB" "--ariaFB_lock_manager=${ariafb_lock_manager}" || failed=1
