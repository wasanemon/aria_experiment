...
# TPCC sweep for abort_lock fix variants.
# Results are stored under a dedicated timestamped directory separate from result_tpcc/.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"

ARIA="${ROOT}/aria_abort_lock_fix/bench_tpcc"
ARIAER="${ROOT}/ariaer_abort_lock_fix/bench_tpcc"
# ARIAER_CACHE_FIX="${ROOT}/ariaer_cache_fix_abort_lock_fix/bench_tpcc"
ARIAER_CACHE_FIX_V2="${ROOT}/ariaer_cache_fix_v2_abort_lock_fix/bench_tpcc"
POSTPROCESS_SCRIPT="${SCRIPT_DIR}/tpcc_logs_to_csv.sh"

RESULTS_ROOT="${RESULTS_ROOT:-${ROOT}/result_abort_lock_fix_tpcc}"
...
queries=(mixed neworder payment)
