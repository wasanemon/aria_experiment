...
namespace aria
{
  ...
  class AriaExecutor : public Worker
  {
  public:
    ...
    AriaExecutor(std::size_t coordinator_id, std::size_t id, DatabaseType &db,
                   ...
                   AriaCommitSharedState &commit_shared_state)
        : Worker(coordinator_id, id), db(db), context(context),
          ...
          commit_shared_state(commit_shared_state),
    ...
    void read_snapshot()
    {
      ...
      for (auto i = id; i < transactions.size(); i += context.worker_num)
      {
        ...
        {
          bool first_creation = (transactions[i] == nullptr);
          auto partition_id = get_partition_id();
          transactions[i] =
              workload.next_transaction(context, partition_id, storages[i]);
          if (first_creation && i < 25) {
            transactions[i]->forced_conflict_key = 0;
            if (i == 24) {
              transactions[i]->sleep_on_read_reserve = true;
            }
          }
        }
        else
        {
          int32_t saved_forced = transactions[i]->forced_conflict_key;
          bool saved_sleep = transactions[i]->sleep_on_read_reserve;
          auto partition_id = get_partition_id();
          transactions[i] =
              workload.next_transaction(context, partition_id, storages[i]);
          transactions[i]->forced_conflict_key = saved_forced;
          transactions[i]->sleep_on_read_reserve = saved_sleep;
        }
    ...
    void first_dependency_check_waw(TransactionType &txn)
    {
      ...
      if (txn.waw)
      {
        commit_shared_state.aborts.mark_abort(txn.tid_offset);
      }
    }
      ...
      for (std::size_t i = 0u; i < readSet.size(); i++)
      {
        ...
        if (partitioner->has_master_partition(partitionId))
        {
          ...
          if (ep == txn.epoch && wts < txn.id && wts != 0)
          {
            ...
            if (writer_offset != static_cast<std::size_t>(-1) &&
                writer_offset < transactions.size())
            {
              writer_alive = !commit_shared_state.aborts.is_aborted(writer_offset);
            }
            if (writer_alive)
            {
              txn.raw = true;
    ...
    void barrier() {
      barrier_wait(commit_shared_state.barrier_count.value,
                   commit_shared_state.barrier_gen.value);
    }
    ...
    void setupHandlers(TransactionType &txn)
    {
      ...
      txn.remote_request_handler = [this]()
