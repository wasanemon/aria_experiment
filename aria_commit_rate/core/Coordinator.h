...
public:
  ...
  void start() {
    ...
    {
      uint64_t total_batch_commit = 0, total_batch_abort = 0,
               total_batch_count = 0;
      for (auto i = 0u; i < workers.size(); i++) {
        total_batch_commit += workers[i]->n_batch_commit.load();
        total_batch_abort += workers[i]->n_batch_abort.load();
        total_batch_count += workers[i]->n_batch_count.load();
      }
      uint64_t total_batch_txns = total_batch_commit + total_batch_abort;
      double avg_batch_commit = total_batch_count > 0
          ? 1.0 * total_batch_commit / total_batch_count : 0.0;
      double avg_batch_abort = total_batch_count > 0
          ? 1.0 * total_batch_abort / total_batch_count : 0.0;
      double commit_rate = total_batch_txns > 0
          ? 1.0 * total_batch_commit / total_batch_txns : 0.0;
      LOG(INFO) << "batch stats: total_batches=" << total_batch_count
                << " avg_batch_commit=" << avg_batch_commit
                << " avg_batch_abort=" << avg_batch_abort
