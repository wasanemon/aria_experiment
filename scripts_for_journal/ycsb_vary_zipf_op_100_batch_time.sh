...
# YCSB sweep for batch-time experiments.
# - threads: 12
# - partitions: 12
...
ARIA="${ROOT}/aria_time_experiment/bench_ycsb"
ARIAER_CACHE_FIX_V2="${ROOT}/ariaer_cache_fix_v2_time_experiment/bench_ycsb"
POSTPROCESS_SCRIPT="${SCRIPT_DIR}/ycsb_logs_to_csv_batch_time.sh"
RESULTS_ROOT="${RESULTS_ROOT:-${ROOT}/result_for_journal/ycsb_vary_zipf_op_100_batch_time}"
...
summarize_dir () {
  ...
  mkdir -p "${target_dir}"
  echo "zipf\tengine\tkeys\trw\tops\tbatch_size\taverage_batch_time_ms" | tee "${summary_file}"
  ...
  for f in "${target_dir}"/*.log; do
    ...
    avg_batch_ms=$(grep -o "average batch time: [0-9.eE+\\-]*" "$f" | tail -1 | awk '{print $4}')
    echo -e "${z}\t${label}\t${k}\t${rw}\t${ops}\t${bs}\t${avg_batch_ms:-NA}" | tee -a "${summary_file}"
  done
  shopt -u nullglob
...
case "${1:-run}" in
  run)
    ...
    failed=0
    run_engine "${ARIA}" "aria_time_experiment" "Aria" || failed=1
    run_engine "${ARIAER_CACHE_FIX_V2}" "ariaer_cache_fix_v2_time_experiment" "Aria" || failed=1
