...
namespace aria
{
  ...
  class AriaManager : public aria::Manager
  {
  public:
    ...
    AriaManager(std::size_t coordinator_id, std::size_t id, DatabaseType &db,
        ...
        : base_type(coordinator_id, id, context, stopFlag), db(db), epoch(0),
          commit_shared_state(context.batch_size)
    {
    ...
    void coordinator_start() override
    {
      ...
      std::chrono::nanoseconds total_batch_duration(0);
      ...
      if (total_batches > 0)
      {
        auto average_batch_time_ms =
            std::chrono::duration_cast<
                std::chrono::duration<double, std::milli>>(
                total_batch_duration)
                .count() /
            total_batches;
        LOG(INFO) << "average batch time: " << average_batch_time_ms
                  << " ms, batch count: " << total_batches;
      }
