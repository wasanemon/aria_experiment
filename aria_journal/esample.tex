...
\title{Optimizing Aria Concurrency Control Protocol\\with Early Dependency Resolution}
...
To address the limited scheduling space caused by Aria's simultaneous dependency checking, we propose a novel optimization method, \textbf{early dependency resolution} (ER) for Aria. 
%
By applying ER, Aria can avoid unnecessary aborts and redundant reservation processing.
%
Hereafter, we refer to Aria augmented with early dependency resolution as \textbf{AriaER}.
%

AriaER separates the dependency checking into two stages to eliminate the false positives caused by simultaneous checking. 
%
...
Second, only those transactions that are not aborted by WAW dependencies in the first stage can perform read reservations and check for WAR and RAW dependencies. Vanilla Aria checks all three dependencies simultaneously, whereas AriaER does not need to recheck WAW dependencies in this stage.
%
...
We show a running example of the behavior of AriaER in Figure~\ref{fig:ariaER_method}.
%
...
The details of AriaER are presented in Algorithms~\ref{alg1},~\ref{alg2}, and~\ref{alg3}. Algorithms~\ref{alg1} and~\ref{alg2} define the subroutines for reservation and dependency checking, corresponding to \maru{1} and \maru{2} in Figure~\ref{fig:ariaER_method}, respectively. Algorithm~\ref{alg3} describes the overall batch processing flow that invokes these subroutines, including the elimination of WAW-aborted transactions.

\begin{figure}[tb] 
\centering
\includegraphics[width=0.95\linewidth]{figures2/edrmethod.pdf}
\caption{Behavior of AriaER. $t_2$ detects a WAW dependency with $t_1$ when it checks item x/write\_tid. Since $t_2$ is recorded as WAW-aborted, its reservations are ignored during subsequent dependency checking. Thus, $t_3$ is not affected by $t_2$.} 
...
\begin{figure}[!t]
\begin{algorithm}[H]
    \caption{First dependency check in AriaER}
    \label{alg1}
    \begin{algorithmic}[1]  
...
\begin{figure}[!t]
\begin{algorithm}[H]
    \caption{Second dependency check in AriaER}
    \label{alg2}
    \begin{algorithmic}[1]  
    \Require Transaction $Tx$ with read set $Tx.RS$, write set $Tx.WS$, transaction id $Tx.TID$, epoch $Tx.Epoch$, abort-list entry $Tx.Offset$; current generation $g$
...
\begin{figure}[!t]
\begin{algorithm}[H]
    \caption{Batch processing in AriaER}
    \label{alg3}
    \begin{algorithmic}[1]
...
For the baseline, we used the open-source implementation of Aria published on GitHub~\cite{lu2020aria_github}. The implementation of AriaER was built upon this codebase with the proposed modifications applied.
...
AriaER is characterized by its ability to exclude, through early dependency resolution, the effects of transactions whose abort has already been determined by WAW dependencies from the later stages of dependency checking. As a result, it is expected to reduce unnecessary false positives that would otherwise abort transactions that could actually commit. In addition, whereas vanilla Aria performs read reservations even for transactions that are eventually aborted, AriaER can eliminate such unnecessary transactions at an earlier stage, thereby reducing redundant read reservations as well.

Therefore, while AriaER is not expected to differ significantly from Aria under low-contention conditions, its benefit should become more apparent under medium- to high-contention workloads, where reducing unnecessary aborts and reservation processing matters more. In particular, we hypothesize that AriaER will achieve higher performance than Aria for read-intensive workloads, because the reduction of redundant read reservations is expected to be especially effective in such settings.

\subsection{Conditions}

We used the YCSB workload for evaluation. Unless otherwise noted, the following parameters are fixed to the same values as in the original paper~\cite{lu2020aria}: \texttt{threads}=12, \texttt{partitions}=12, \texttt{keys}=40000, \texttt{cross\_ratio}=100, and \texttt{global\_key\_space}=True. In each experiment, we vary only the parameters of interest---\texttt{batch\_size}, \texttt{ops\_per\_txn}, \texttt{read\_write\_ratio}, and the Zipf coefficient---to control contention and transaction characteristics. A higher Zipf coefficient concentrates key accesses on a smaller subset of records, thereby increasing contention among transactions.

We first evaluated AriaER under medium- to high-contention conditions with long transactions, where the advantages of early dependency resolution were expected to be most pronounced (Section~4.4). We then varied the batch size to examine how the throughput gap scales with the number of transactions per batch (Section~4.5), and analyzed commit rates and batch execution times to disentangle the two sources of improvement: suppression of unnecessary aborts and elimination of redundant processing (Section~4.6). To confirm that AriaER does not degrade performance under low-contention conditions, we also compared the two methods under the same configuration as the original paper (Section~4.7). We further evaluated both methods on the TPC-C benchmark to examine their behavior on a more realistic, multi-table workload (Section~4.8). Finally, we designed a controlled experiment that artificially amplifies the cost of redundant read reservations, directly demonstrating the core mechanism behind AriaER's advantage (Section~4.9).
...
We first set the number of operations per transaction to 100, which broadens the access range and increases the probability of conflicts between transactions, thereby amplifying the effect of AriaER's early dependency resolution.

Figure~\ref{fig:medium_high} shows the results. AriaER achieved throughput equal to or higher than Aria across all Zipf coefficients. As the Zipf coefficient increased, the throughput gap widened, peaking at approximately 2.4$\times$ near Zipf$\,{=}\,$0.8. Beyond that point the ratio declined, yet AriaER remained consistently superior.

Under moderate contention, WAW dependencies occur frequently enough for AriaER's early detection to effectively suppress false-positive aborts in the subsequent dependency checking stages. Under extremely high contention, however, aborts caused by RAW and WAR dependencies become dominant, and early WAW detection alone cannot absorb all the overhead. Furthermore, the absolute throughput of both methods drops substantially under extreme contention, which narrows the relative ratio.
...
\begin{figure}[tb]
\centering
\includegraphics[width=\linewidth]{figure_pdf/vary_batch_ratio.pdf}
\caption{Throughput ratio (AriaER/Aria) as a function of batch size (\texttt{read\_write\_ratio}=95, \texttt{ops\_per\_txn}=100, Zipf$\,{=}\,$0.9).}
\label{fig:vary_batch_ratio}
\end{figure}

Next, we varied the batch size from 1\,000 to 10\,000 in increments of 1\,000 to investigate how the throughput gap between Aria and AriaER changes as the batch grows larger. Wang et al.~\cite{10.1007/s11704-023-2605-z} pointed out that configuring an appropriate batch size in Aria is a practical challenge, because the optimal batch size depends on workload characteristics: a large batch size suits low-contention workloads, whereas a small batch size suits high-contention workloads. Evaluating AriaER across a range of batch sizes is therefore essential not only to demonstrate its robustness under diverse configurations, but also to provide insights for future algorithmic extensions such as adaptive batch sizing.

We set \texttt{read\_write\_ratio}=95, \texttt{ops\_per\_txn}=100, and Zipf$\,{=}\,$0.9, choosing a high read ratio to amplify the effect of AriaER's elimination of unnecessary read reservations.

Figures~\ref{fig:vary_batch} and~\ref{fig:vary_batch_ratio} show the results. The throughput ratio (AriaER/Aria) increased monotonically with batch size, reaching approximately 2.2$\times$ at batch size$\,{=}\,$10\,000. In Aria, the cumulative cost of read reservations and dependency checking for transactions that should be aborted grows as the number of transactions in a batch increases. AriaER skips these downstream operations for transactions whose abort has been determined by early WAW detection, so the benefit of this elimination accumulates further as the batch grows larger.
...
\caption{Average execution time per batch for Aria and AriaER (\texttt{ops\_per\_txn}=100, \texttt{batch\_size}=1000, \texttt{read\_write\_ratio}=80). Each batch is defined as one full iteration of the \texttt{AriaManager} loop, measured as wall-clock time from the start of \texttt{cleanup\_batch()} to the completion of the final synchronization after the \texttt{Aria\_COMMIT} phase.}
\label{fig:batch_time}
\end{figure}

\begin{figure}[tb]
\centering
\includegraphics[width=\linewidth]{figure_pdf/batch_time_diff.pdf}
\caption{Difference in average batch execution time between Aria and AriaER (Aria $-$ AriaER). A positive value indicates that AriaER processes each batch faster than Aria.}
\label{fig:batch_time_diff}
\end{figure}

Combining the results from the preceding sections, the throughput improvement of AriaER can be attributed not merely to an increase in the number of committed transactions, but to two underlying effects: the suppression of unnecessary aborts and the elimination of redundant processing for transactions whose abort has already been determined. In this section, we quantitatively analyze these two effects from the perspectives of the commit rate and batch execution time.

\subsubsection{Commit Rate Comparison}

Figure~\ref{fig:commit_rate} shows the commit rate---defined as the ratio of committed transactions to the batch size---with \texttt{ops\_per\_txn}=100, \texttt{batch\_size}=1000, and \texttt{read\_write\_ratio}=80 while varying the Zipf coefficient. This metric directly represents how large a fraction of the scheduling space each protocol can successfully exploit per batch; a higher commit rate indicates that a wider range of the scheduling space is utilized.

AriaER achieved a higher commit rate than Aria across all Zipf coefficients. As contention increased, both commit rates declined, but AriaER consistently maintained a higher value, committing up to approximately twice as many transactions per batch as Aria under high contention. Under extremely high contention, the gap narrowed because the vast majority of transactions are aborted due to WAW dependencies, leaving very few surviving transactions that can benefit from the expanded scheduling space.

These results demonstrate that AriaER expands the effective scheduling space of Aria. In vanilla Aria, transactions that should be aborted due to WAW dependencies continue to hold their read reservations, which in turn trigger false-positive aborts of other transactions during the subsequent RAW/WAR checking. AriaER detects WAW dependencies early and excludes the affected transactions from downstream processing, thereby suppressing this cascading abort effect and allowing more transactions per batch to commit successfully.

\subsubsection{Batch Execution Time Comparison}

Figure~\ref{fig:batch_time} shows the average execution time per batch, measured using a dedicated time-instrumented implementation. One batch is defined as a single iteration of the \texttt{AriaManager} loop, i.e., the wall-clock time from the start of \texttt{cleanup\_batch()} to the completion of the final synchronization (\texttt{wait4\_ack()}) after the \texttt{Aria\_COMMIT} phase.

AriaER exhibited shorter batch execution times than Aria across all Zipf coefficients. As shown in Figure~\ref{fig:batch_time_diff}, the time savings grew with contention, reaching a reduction of approximately 7\% under high contention. This demonstrates that the advantage of AriaER is not solely a consequence of fewer aborts leading to higher throughput; rather, by skipping read reservations and dependency resolution for transactions whose abort has already been determined, AriaER reduces the processing time of each batch itself.

In summary, the performance improvement of AriaER is rooted in two synergistic effects: (1)~the expansion of the scheduling space through early dependency resolution, as demonstrated by the higher commit rate, and (2)~the elimination of downstream processing (read reservations and dependency checking) for transactions whose abort has been determined. These effects become more pronounced as contention increases, and as confirmed in Section~4.5, they accumulate further as the batch size grows larger.

%%% ---------- Comparison under original paper conditions ----------
\subsection{Comparison Under Original Paper Conditions}

\begin{figure}[tb]
\centering
\includegraphics[width=\linewidth]{figure_pdf/same_condition.pdf}
\caption{Throughput comparison under the same conditions as the original paper (\texttt{read\_write\_ratio}=80, \texttt{ops\_per\_txn}=10, \texttt{batch\_size}=1000).}
\label{fig:same_condition}
\end{figure}

Finally, we compare throughput under the same experimental settings as the original paper~\cite{lu2020aria}: \texttt{ops\_per\_txn}=10, \texttt{batch\_size}=1000, and \texttt{read\_write\_ratio}=80. In this setting, each transaction performs only 10 operations, resulting in lighter contention compared to the \texttt{ops\_per\_txn}=100 condition used in the preceding sections.

Figure~\ref{fig:same_condition} shows the results. Under low-to-moderate contention, the throughput of Aria and AriaER was nearly identical. Even under high contention, the difference remained marginal, confirming that AriaER introduces no measurable overhead in this setting.

This result is consistent with the hypothesis. When the number of operations per transaction is small, the access range within a batch is narrow, and the probability of WAW dependencies is low. Consequently, AriaER's early dependency resolution has limited opportunity to take effect, and the performance difference between the two methods is minimal. In other words, AriaER maintains performance equivalent to Aria under the original paper's conditions, while achieving substantial improvements under high-contention, long-transaction conditions.

%%% ---------- TPC-C benchmark evaluation ----------
\subsection{Evaluation on TPC-C Workload}

\begin{figure}[tb]
\centering
\includegraphics[width=\linewidth]{figure_pdf/tpcc_mixed.pdf}
\caption{Throughput comparison on TPC-C mixed workload (\texttt{batch\_size}=1000, \texttt{threads}=12, \texttt{n\_district}=10). The x-axis represents the number of warehouses (partitions), and the y-axis represents throughput.}
\label{fig:tpcc_mixed}
\end{figure}

To examine AriaER's behavior on a more realistic workload beyond YCSB, we evaluated both methods on the TPC-C benchmark with mixed queries (New-Order and Payment). We fixed \texttt{batch\_size}=1000, \texttt{threads}=12, and \texttt{n\_district}=10, and varied the number of warehouses from 1 to 180. In TPC-C, a smaller number of warehouses concentrates data accesses and increases contention, analogously to a higher Zipf coefficient in YCSB.

Figure~\ref{fig:tpcc_mixed} shows the results. Across all warehouse counts, the throughput of Aria and AriaER was nearly identical, with the ratio ranging from approximately 0.96$\times$ to 1.07$\times$. Both methods exhibit monotonically increasing throughput as the number of warehouses grows, because more warehouses spread data accesses across a larger key space and reduce inter-transaction conflicts. With fewer warehouses, contention on shared data items such as the \texttt{d\_ytd} field in the district table increases the frequency of WAW dependencies, and AriaER's early dependency resolution can take slight effect. This result confirms that AriaER introduces no measurable overhead on TPC-C and behaves comparably to Aria on a multi-table transactional workload.
...
The preceding sections have shown that AriaER improves throughput by eliminating redundant read reservations for WAW-aborted transactions.
...
In Aria, all transactions---including those that will ultimately be WAW-aborted---execute read reservations in every batch. Therefore, the 1-second sleep is incurred in \emph{every} batch until the 25th transaction commits, yielding a cumulative delay of approximately 25~seconds. In AriaER, by contrast, transactions whose abort has been determined by early WAW detection skip the read-reservation phase entirely. The sleep is thus triggered only in the single batch where the 25th transaction finally commits, resulting in a one-time 1-second delay.

\subsubsection{Results}
Figure~\ref{fig:sleep_throughput} shows the throughput of Aria and AriaER in this experiment. Aria's throughput was limited to roughly one thousand commits per second across all Zipf coefficients, confirming that the 1-second sleep dominates the batch duration. AriaER, by contrast, achieved throughput on the order of several hundred thousand commits per second, since the sleep was incurred only once during the entire run. The resulting throughput ratio reached up to approximately 774$\times$.

This result directly confirms AriaER's core mechanism: by detecting WAW dependencies early, AriaER skips read reservations and dependency checking for already-doomed transactions, avoiding the repeated cost that Aria must bear in every batch. In general, as the per-transaction cost of read reservations grows---due to a larger number of read operations or heavier processing per operation---the benefit of this early elimination becomes more pronounced.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Conclusion}
Aria, a deterministic concurrency control protocol, generates unnecessary aborts and redundant operations because it checks all dependencies at once. This paper proposed AriaER, an optimization that reduces these inefficiencies through early dependency resolution, which divides dependency checking into two stages. Experiments under high contention showed that AriaER achieved higher throughput than Aria. Furthermore, in experiments with long-running transactions, AriaER achieved up to 774$\times$ higher throughput than Aria. A controlled experiment that artificially amplified the cost of a single redundant read reservation confirmed that AriaER's advantage stems from skipping subsequent operations for WAW-aborted transactions. These results indicate that AriaER is less susceptible to delays caused by aborted transactions with long read operations, and that the benefit of early dependency resolution grows with the per-transaction cost of redundant operations.
